...
def load_employee_data(config: VoucherConfig, *, usecols: Optional[Iterable[str]] = None) -> pd.DataFrame:
	file_path = config.data_dir / config.employee_file
	df = pd.read_excel(file_path, header=0, engine="openpyxl", dtype=str, usecols=usecols)
	df.columns = [str(col).strip() for col in df.columns]
	return df
...
def _build_employee_map(df: pd.DataFrame, config: VoucherConfig) -> Dict[str, EmployeeInfo]:
	...
	for _, row in df.iterrows():
		info = EmployeeInfo(
			...
			english_name=str(row.get(config.employee_english_name_col, "")).strip() or None,
		)
...
def _resolve_amount_columns(expense_df: pd.DataFrame, config: VoucherConfig) -> List[str]:
...
def _map_subject(expense_col: str, row: pd.Series, subject_map: Dict[str, str], config: VoucherConfig) -> Optional[str]:
...
def _compose_summary(
	...
	en_parts = [part for part in ((english_name or "").strip(), english_desc.strip()) if part]
	...
	*,
	...
	for _, row in expense_df.iterrows():
		for column in amount_columns:
			...
			if amount is None:
				continue
			chinese_desc = str(row.get(cfg.expense_summary_col, "")).strip() or str(column).strip()
			if chinese_desc:
	...
	def _derive_period() -> Tuple[int, int]:
		if raw_period and len(raw_period) == 6 and raw_period.isdigit():
			...
			if 1 <= month <= 12:
				return year, month
		raise ValueError("\u65e0\u6cd5\u4ece\u8d39\u7528\u8868\u5de5\u4f5c\u8868\u540d\u6216\u914d\u7f6e\u4e2d\u89e3\u6790\u4f1a\u8ba1\u671f\u95f4\uff08\u9700\u8981 yyyymm \u683c\u5f0f\uff09")
	...
	for _, row in _iter_rows_with_progress(expense_df, "\u751f\u6210\u51ed\u8bc1"):
		...
		for column in amount_columns:
			...
			summary = _compose_summary(name_value, chinese_desc, english_name_value, english_desc)
