...
# 时间节约估算（单位：分钟/条记录）
# 基于人工半自动化处理（Excel公式+人工）与全自动化处理的差异
TIME_SAVING_RATES = {
    "费用清单转凭证": 2.0,  # 每条记录节约2分钟
    # 人工方式：VLOOKUP查员工+公式匹配科目+批量填充+人工校对翻译+核对平衡
    # 自动化节省：省去公式编写、批量复制、格式调整、人工翻译、错误检查等

    "摘要翻译": 1.0,  # 每条翻译节约1分钟
    # 人工方式：在线翻译工具批量+专业术语人工校对+格式调整
    # 自动化节省：省去复制粘贴、术语查找、格式统一等

    "银行流水转凭证": 3.0,  # 每条流水节约3分钟
    # 人工方式：Excel公式初步匹配+IF判断借贷+VLOOKUP三重映射+人工处理特殊情况
    # 自动化节省：省去复杂公式编写、多表查找、数据清洗、异常处理等
}
