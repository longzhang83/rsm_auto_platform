...
class WeWorkService:
    ...
    def login_or_create_user(cls, db: Session, wework_info: Dict[str, any]) -> Token:
        ...
        2. 优先通过邮箱用户名部分匹配（最可靠，因为邮箱都是英文名）
           例如：zhanglong@wework.local 和 zhanglong@rsmchina.com.cn
        3. 备选通过用户名匹配（如果用户注册时使用中文名）
        4. 否则创建新用户
